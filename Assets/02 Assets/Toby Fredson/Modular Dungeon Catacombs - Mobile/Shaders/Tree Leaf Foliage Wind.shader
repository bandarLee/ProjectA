...
Shader "Tobyfredson/Tree Leaf Foliage Wind"
{
	Properties
	{
		...
		[Header(_____________________________________________________)][Header(Material Settings)][Header((Alpha Clip))]_AlphaCutout("Alpha Cutout", Range( 0 , 1)) = 0.4
		...
		[ASEEnd][Toggle(_WINDONOFF_ON)] _WindOnOff("Wind On/Off", Float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float clampResult330 = clamp( inputMesh.positionOS.y , -0.9 , 1.2 );
				float clampResult318 = clamp( inputMesh.positionOS.x , -0.3 , 0.3 );
				float clampResult314 = clamp( inputMesh.positionOS.z , -1.5 , 1.5 );
				float clampResult343 = clamp( ( clampResult330 * ( ( clampResult318 * ( clampResult314 * ( 0.25 * (inputMesh.positionOS).x ) ) ) * 0.002 ) ) , -1.0 , 1.0 );
				...
				float gradientNoise274 = GradientNoise((ase_worldPos*1.0 + float3( ( _TimeParameters.x * float2( 0,-0.1 ) ) ,  0.0 )).xy,20.0);
				gradientNoise274 = gradientNoise274*0.5 + 0.5;
				float4 temp_cast_7 = (gradientNoise274).xxxx;
				float4 appendResult301 = (float4(( inputMesh.positionOS.y * simplePerlin2D284 ) , ( ( CalculateContrast(_WindAngley,temp_cast_7) * 0.25 ) + ( inputMesh.positionOS.y * 1.2 ) ).r , inputMesh.positionOS.z , 0.0));
				float4 lerpResult307 = lerp( float4( inputMesh.positionOS , 0.0 ) , appendResult301 , inputMesh.ase_color.g);
				float2 texCoord292 = inputMesh.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float4 FinalWind344 = ( 100.0 * ( ( _GlobalWindPower * ( float4( inputMesh.positionOS , 0.0 ) - lerpResult307 ) ) * ( inputMesh.ase_color.g * ( pow( abs( ( texCoord292.y * 0.5 ) ) , ( 1.0 - (0.0 + (_WindPower - 0.0) * (0.89 - 0.0) / (1.0 - 0.0)) ) ) * simplePerlin2D309 ) ) ) );
				float3 _Vector1 = float3(0,0,0);
				float4 temp_cast_9 = (_Vector1.y).xxxx;
				float4 localWorldVar627 = float4( ase_worldPos , 0.0 );
				(localWorldVar627).xyz = GetCameraRelativePositionWS((localWorldVar627).xyz);
				float4 transform627 = mul(GetWorldToObjectMatrix(),localWorldVar627);
				float4 appendResult640 = (float4(( ( inputMesh.positionOS.y * cos( ( ( ( transform627.x + transform627.z ) * _WorldFrequency ) + ( _TimeParameters.x ) ) ) ) * _BendAmount ) , _Vector1.y , 0.0 , 0.0));
				float clampResult641 = clamp( ( transform627.y * 10.0 ) , 10.0 , 10.0 );
				float4 lerpResult643 = lerp( temp_cast_9 , appendResult640 , clampResult641);
				float3 rotatedValue646 = RotateAroundAxis( float3( 0,0,0 ), ( FinalWind344 + lerpResult643 ).xyz, temp_cast_0, _WindDirection );
				#ifdef _WINDONOFF_ON
				float3 staticSwitch441 = rotatedValue646;
				#else
				float3 staticSwitch441 = float3( 0,0,0 );
				#endif
				float3 TrunkPivotBend439 = staticSwitch441;
				
				float3 objectSpaceLightDir = mul( GetWorldToObjectMatrix(), SafeNormalize(-_DirectionalLightDatas[0].forward) ).xyz;
				#ifdef _MOBILESHADINGBACKFACELIGHT_ON
				float3 staticSwitch449 = objectSpaceLightDir;
				#else
				float3 staticSwitch449 = inputMesh.normalOS;
				#endif
				...
				inputMesh.normalOS = staticSwitch449;
						...
						)
			{
				...
				float2 uv_AlbedoMap426 = packedInput.ase_texcoord5.xy;
				float4 tex2DNode426 = tex2D( _AlbedoMap, uv_AlbedoMap426 );
				float4 break382 = tex2DNode426;
				float3 objToWorld653 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( float3( 0,0,0 ), 1 ) ).xyz);
				float dotResult4_g18 = dot( objToWorld653.xy , float2( 12.9898,78.233 ) );
				...
				#endif
				float4 temp_output_377_0 = ( staticSwitch369 * _AlbedoLightness * _AlbedoColor );
				float3 objToWorldDir379 = normalize( mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord6.xyz, 0 ) ).xyz );
				float dotResult366 = dot( V , -( SafeNormalize(-_DirectionalLightDatas[0].forward) + ( objToWorldDir379 * _TranslucencyRange ) ) );
				float2 uv_MaskMap593 = packedInput.ase_texcoord5.xy;
				float TranslucencyBase406 = ( saturate( dotResult366 ) * tex2D( _MaskMap, uv_MaskMap593 ).b );
				float TranslucencyPower471 = (1.0 + (_TranslucencyPower - 0.0) * (5.0 - 1.0) / (1.0 - 0.0));
				
				float2 uv_NormalMap386 = packedInput.ase_texcoord5.xy;
				...
				float3 appendResult367 = (float3(tex2DNode386.r , tex2DNode386.g , ( tex2DNode386.b * isFrontFace )));
				...
				float2 uv_MaskMap415 = packedInput.ase_texcoord5.xy;
				float4 tex2DNode415 = tex2D( _MaskMap, uv_MaskMap415 );
				float SmoothnessOutput403 = ( tex2DNode415.a * _SmoothnessIntensity * isFrontFace );
				
				float2 uv_MaskMap458 = packedInput.ase_texcoord5.xy;
				...
				float3 bakedGI622 = ASEBakedGI( float3( 0,0,0 ), float3( 0,0,0 ), float2( 0,0 ), float2( 0,0 ) );
				
				surfaceDescription.BaseColor = ( temp_output_377_0 * (1.0 + (TranslucencyBase406 - 0.0) * (TranslucencyPower471 - 1.0) / (1.0 - 0.0)) ).rgb;
				surfaceDescription.Normal = NormalOutput402;
				...
				surfaceDescription.Smoothness = SmoothnessOutput403;
				surfaceDescription.Occlusion = AOOutput461;
				...
				#ifdef ASE_BAKEDGI
				surfaceDescription.BakedGI = bakedGI622;
				#endif
				#ifdef ASE_BAKEDBACKGI
				surfaceDescription.BakedBackGI = ( float3( 0,0,0 ) * bakedGI622 );
				#endif
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				float4 appendResult301 = (float4(( inputMesh.positionOS.y * simplePerlin2D284 ) , ( ( CalculateContrast(_WindAngley,temp_cast_7) * 0.25 ) + ( inputMesh.positionOS.y * 1.2 ) ).r , inputMesh.positionOS.z , 0.0));
				float4 lerpResult307 = lerp( float4( inputMesh.positionOS , 0.0 ) , appendResult301 , inputMesh.ase_color.g);
				float2 texCoord292 = inputMesh.uv0.xy * float2( 1,1 ) + float2( 0,0 );
				float2 appendResult289 = (float2(ase_worldPos.x , ase_worldPos.z));
				float2 appendResult294 = (float2(_TimeParameters.x , _TimeParameters.x));
				float simplePerlin2D309 = snoise( ( appendResult289 + appendResult294 )*_WindSpeed );
				simplePerlin2D309 = simplePerlin2D309*0.5 + 0.5;
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_AlbedoMap426 = packedInput.ase_texcoord2.xy;
				float4 tex2DNode426 = tex2D( _AlbedoMap, uv_AlbedoMap426 );
				float4 break382 = tex2DNode426;
				float3 objToWorld653 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( float3( 0,0,0 ), 1 ) ).xyz);
				float dotResult4_g18 = dot( objToWorld653.xy , float2( 12.9898,78.233 ) );
				...
				float3 objToWorldDir379 = normalize( mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord4.xyz, 0 ) ).xyz );
				float dotResult366 = dot( ase_worldViewDir , -( SafeNormalize(-_DirectionalLightDatas[0].forward) + ( objToWorldDir379 * _TranslucencyRange ) ) );
				float2 uv_MaskMap593 = packedInput.ase_texcoord2.xy;
				float TranslucencyBase406 = ( saturate( dotResult366 ) * tex2D( _MaskMap, uv_MaskMap593 ).b );
				float TranslucencyPower471 = (1.0 + (_TranslucencyPower - 0.0) * (5.0 - 1.0) / (1.0 - 0.0));
				
				float2 uv_NormalMap386 = packedInput.ase_texcoord2.xy;
				float3 unpack386 = UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap386 ), _NormalIntensity );
				unpack386.z = lerp( 1, unpack386.z, saturate(_NormalIntensity) );
				float3 tex2DNode386 = unpack386;
				float3 appendResult367 = (float3(tex2DNode386.r , tex2DNode386.g , ( tex2DNode386.b * isFrontFace )));
				...
				float2 uv_MaskMap415 = packedInput.ase_texcoord2.xy;
				float4 tex2DNode415 = tex2D( _MaskMap, uv_MaskMap415 );
				float SmoothnessOutput403 = ( tex2DNode415.a * _SmoothnessIntensity * isFrontFace );
				
				float2 uv_MaskMap458 = packedInput.ase_texcoord2.xy;
				#ifdef _TRANSLUCENCYOCCLUSION_ON
				float staticSwitch465 = ( 1.0 / ( ( saturate( TranslucencyBase406 ) * TranslucencyPower471 ) + 1.0 ) );
				#else
				float staticSwitch465 = 1.0;
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_AlbedoMap426 = packedInput.ase_texcoord1.xy;
				float4 tex2DNode426 = tex2D( _AlbedoMap, uv_AlbedoMap426 );
				float OpacityMaskOutput359 = tex2DNode426.a;
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_NormalMap386 = packedInput.ase_texcoord3.xy;
				float3 unpack386 = UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap386 ), _NormalIntensity );
				unpack386.z = lerp( 1, unpack386.z, saturate(_NormalIntensity) );
				float3 tex2DNode386 = unpack386;
				float3 appendResult367 = (float3(tex2DNode386.r , tex2DNode386.g , ( tex2DNode386.b * isFrontFace )));
				...
				float2 uv_MaskMap415 = packedInput.ase_texcoord3.xy;
				float4 tex2DNode415 = tex2D( _MaskMap, uv_MaskMap415 );
				float SmoothnessOutput403 = ( tex2DNode415.a * _SmoothnessIntensity * isFrontFace );
				
				float2 uv_AlbedoMap426 = packedInput.ase_texcoord3.xy;
				float4 tex2DNode426 = tex2D( _AlbedoMap, uv_AlbedoMap426 );
				float OpacityMaskOutput359 = tex2DNode426.a;
		...
		Pass
		{
				...
				)
			{
				...
				float2 uv_MaskMap415 = packedInput.ase_texcoord3.xy;
				float4 tex2DNode415 = tex2D( _MaskMap, uv_MaskMap415 );
				float SmoothnessOutput403 = ( tex2DNode415.a * _SmoothnessIntensity * ase_vface );
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_AlbedoMap426 = packedInput.ase_texcoord7.xy;
				float4 tex2DNode426 = tex2D( _AlbedoMap, uv_AlbedoMap426 );
				float4 break382 = tex2DNode426;
				float3 objToWorld653 = GetAbsolutePositionWS(mul( GetObjectToWorldMatrix(), float4( float3( 0,0,0 ), 1 ) ).xyz);
				float dotResult4_g18 = dot( objToWorld653.xy , float2( 12.9898,78.233 ) );
				...
				#endif
				float4 temp_output_377_0 = ( staticSwitch369 * _AlbedoLightness * _AlbedoColor );
				float3 objToWorldDir379 = normalize( mul( GetObjectToWorldMatrix(), float4( packedInput.ase_texcoord8.xyz, 0 ) ).xyz );
				float dotResult366 = dot( V , -( SafeNormalize(-_DirectionalLightDatas[0].forward) + ( objToWorldDir379 * _TranslucencyRange ) ) );
				float2 uv_MaskMap593 = packedInput.ase_texcoord7.xy;
				float TranslucencyBase406 = ( saturate( dotResult366 ) * tex2D( _MaskMap, uv_MaskMap593 ).b );
				float TranslucencyPower471 = (1.0 + (_TranslucencyPower - 0.0) * (5.0 - 1.0) / (1.0 - 0.0));
				
				float2 uv_NormalMap386 = packedInput.ase_texcoord7.xy;
				float3 unpack386 = UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap386 ), _NormalIntensity );
				unpack386.z = lerp( 1, unpack386.z, saturate(_NormalIntensity) );
				float3 tex2DNode386 = unpack386;
				float3 appendResult367 = (float3(tex2DNode386.r , tex2DNode386.g , ( tex2DNode386.b * isFrontFace )));
				...
				float2 uv_MaskMap415 = packedInput.ase_texcoord7.xy;
				float4 tex2DNode415 = tex2D( _MaskMap, uv_MaskMap415 );
				float SmoothnessOutput403 = ( tex2DNode415.a * _SmoothnessIntensity * isFrontFace );
				
				float2 uv_MaskMap458 = packedInput.ase_texcoord7.xy;
				#ifdef _TRANSLUCENCYOCCLUSION_ON
				float staticSwitch465 = ( 1.0 / ( ( saturate( TranslucencyBase406 ) * TranslucencyPower471 ) + 1.0 ) );
				#else
				float staticSwitch465 = 1.0;
...
Node;AmplifyShaderEditor.RangedFloatNode;272;-3716.156,1133.34;Inherit;False;Property;_WindAngley;Wind Angle (y);26;0;Create;True;0;0;0;True;0;False;20;100;-100;100;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;341;-3280.066,-480.1826;Inherit;False;2724.468;1204.099;;41;433;431;430;429;428;426;416;415;414;413;412;411;410;409;408;407;404;403;402;401;393;386;384;378;377;376;374;373;372;371;369;367;365;361;359;472;473;598;599;600;603;Base Inputs;1,1,1,1;0;0
...
Node;AmplifyShaderEditor.RangedFloatNode;348;-2039.017,4830.763;Inherit;False;Property;_BranchBending;Branch Bending;27;0;Create;True;0;0;0;False;0;False;1;1;0;10;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.CommentaryNode;357;-2841.219,2431.643;Inherit;False;1626.367;646.2914;Comment;15;406;593;594;389;432;471;405;392;387;385;381;379;366;362;595;Translucency Base;1,1,1,1;0;0
Node;AmplifyShaderEditor.CommentaryNode;358;-1544.76,1959.857;Inherit;False;922.786;393.881;Comment;7;396;606;605;380;607;609;375;Vertex AO;1,1,1,1;0;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;359;-852.1114,278.3641;Inherit;False;OpacityMaskOutput;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.CommentaryNode;360;-4419.239,-488.7567;Inherit;False;1047.931;504.8553;Comment;10;400;398;397;395;383;382;370;368;363;653;Grass Color Variation;0.7504205,1,0,1;0;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;373;-2174.232,-290.3311;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.PowerNode;393;-2661.583,513.321;Inherit;False;False;2;0;FLOAT;0;False;1;FLOAT;1;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;402;-843.5239,163.5268;Inherit;False;NormalOutput;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;403;-844.1782,536.4612;Inherit;False;SmoothnessOutput;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.WireNode;404;-2674.666,-171.2077;Inherit;False;1;0;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.SaturateNode;405;-1972.74,2615.415;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;407;-838.4341,429.03;Inherit;False;TranslucencyOutput;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;408;-2137.931,553.4341;Inherit;False;406;TranslucencyBase;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;409;-1662.245,223.3973;Inherit;False;5;5;0;FLOAT;0;False;1;FLOAT;0;False;2;COLOR;0,0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.LightColorNode;410;-1945.01,345.2526;Inherit;False;0;3;COLOR;0;FLOAT3;1;FLOAT;2
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;411;-1449.075,321.7045;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.FaceVariableNode;413;-2379.029,551.4716;Inherit;False;0;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;415;-3058.282,315.0198;Inherit;True;Property;_MaskMap;Mask Map;2;1;[NoScaleOffset];Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.AbsOpNode;416;-2776.398,505.0508;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;431;-2946.241,582.3251;Inherit;False;Property;_AmbientOcclusion;Ambient Occlusion;12;1;[Header];Create;True;1;(Ambient Occlusion);0;0;False;0;False;1;1;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;434;-2105.177,982.1667;Inherit;False;Property;_GlobalWindPower;Global Wind Power;20;1;[Header];Create;True;3;_____________________________________________________;Wind Settings;(Vertex Offset);0;0;False;0;False;1;1.2;0;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;435;-3082.313,1956.481;Inherit;False;Property;_WindPower;Wind Power;24;0;Create;True;0;0;0;False;0;False;1;0.89;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;437;-2622.11,2070.531;Inherit;False;Property;_WindSpeed;Wind Speed;25;0;Create;True;0;0;0;False;0;False;1;1;0;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;471;-2176.076,2896.854;Inherit;False;TranslucencyPower;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;472;-1643.997,453.4995;Inherit;False;471;TranslucencyPower;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;412;-1592.939,-262.3363;Inherit;False;AlbedoOutput;-1;True;1;0;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleAddOpNode;414;-1993.89,-178.3584;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.StaticSwitch;433;-1878.76,-308.6041;Inherit;False;Property;_TranslucencyOnOff;Translucency On/Off;18;0;Create;True;0;0;0;False;0;False;0;1;1;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.CommentaryNode;473;-1358.217,-404.6257;Inherit;False;784.5179;428.8177;Comment;5;479;477;476;475;474;Albedo Output;1,1,1,1;0;0
Node;AmplifyShaderEditor.CommentaryNode;474;-1060.959,-354.6257;Inherit;False;257;257;Comment;1;478;Translucency Albedo;1,1,1,1;0;0
Node;AmplifyShaderEditor.TFHCRemapNode;478;-1010.959,-304.626;Inherit;False;5;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;1;False;4;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;476;-1308.217,-339.4666;Inherit;False;406;TranslucencyBase;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;477;-1316.084,-218.9751;Inherit;False;471;TranslucencyPower;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;479;-1278.895,-106.9996;Inherit;False;-1;;1;0;OBJECT;;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;377;-2263.179,-163.2464;Inherit;False;3;3;0;COLOR;0,0,0,0;False;1;FLOAT;0;False;2;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.ObjSpaceLightDirHlpNode;448;-520.9133,780.1975;Inherit;False;1;0;FLOAT;0;False;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;475;-756.5589,-105.8999;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;FLOAT;0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;594;-1589.279,2755.081;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;593;-1966.985,2698.489;Inherit;True;Property;_TextureSample0;Texture Sample 0;2;0;Create;True;0;0;0;False;0;False;-1;None;None;True;0;False;white;Auto;False;Instance;415;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.TFHCRemapNode;595;-2411.944,2896.292;Inherit;False;5;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;1;False;4;FLOAT;5;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.WireNode;598;-967.0345,118.8312;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;603;-1541.83,-99.04956;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;374;-2496.601,355.8165;Inherit;False;AoBase;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;380;-1513.425,2203.957;Inherit;False;Property;_VertexAointensity;Vertex Ao intensity;15;0;Create;True;0;0;0;False;0;False;0;0.2;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.PowerNode;607;-1205.284,2196.997;Inherit;False;False;2;0;FLOAT;0;False;1;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;606;-1229.589,2015.307;Inherit;False;Constant;_Float4;Float 4;32;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.AbsOpNode;609;-1305.26,2127.592;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;396;-1040.838,2107.976;Inherit;False;Property;_VertexAo;Vertex Ao;14;0;Create;True;0;0;0;False;0;False;0;1;1;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.VertexColorNode;605;-1465.589,2025.307;Inherit;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;470;-2344.407,5312.997;Inherit;False;Property;_AmbientOcclusion1;Ambient Occlusion;13;1;[Header];Create;True;1;(Ambient Occlusion);0;0;False;0;False;1;1;0;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;375;-817.6514,2123;Inherit;False;VertexAo;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;427;-353.8669,387.2923;Inherit;False;Property;_AlphaCutout;Alpha Cutout;3;1;[Header];Create;True;3;_____________________________________________________;Material Settings;(Alpha Clip);0;0;False;0;False;0.4;0.4;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.CommentaryNode;621;-471.5673,952.0069;Inherit;False;467.5709;233;!Fix translucency glow!;2;623;622;;1,1,1,1;0;0
Node;AmplifyShaderEditor.BakedGINode;622;-421.5673,1002.007;Inherit;False;True;4;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT2;0,0;False;3;FLOAT2;0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;623;-165.9964,1021.641;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.StaticSwitch;449;-272.9598,731.729;Inherit;False;Property;_MobileShadingBackfaceLight;Mobile Shading (Back face Light);8;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.CommentaryNode;626;-4090.276,3129.313;Inherit;False;3322.445;798.5574;Comment;25;649;636;624;439;441;642;625;646;645;644;643;641;640;639;638;637;635;634;633;632;631;630;629;628;627;Trunk Pivot Bend;1,1,1,1;0;0
Node;AmplifyShaderEditor.WorldToObjectTransfNode;627;-3852.049,3491.845;Inherit;False;1;0;FLOAT4;0,0,0,1;False;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleAddOpNode;628;-3659.855,3505.771;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;629;-3799.165,3674.531;Inherit;False;Property;_WorldFrequency;World Frequency;22;0;Create;True;0;0;0;False;0;False;1;0.08;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.TimeNode;630;-3427.693,3682.412;Inherit;False;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;631;-3440.866,3456.456;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;632;-3218.028,3554.456;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.CosOpNode;633;-3081.594,3555.997;Inherit;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;634;-3080.439,3784.042;Inherit;False;Property;_BendAmount;Bend Amount;23;0;Create;True;0;0;0;False;0;False;1;1;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;635;-2895.475,3460.382;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;637;-2671.968,3171.671;Inherit;True;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector3Node;638;-2612.075,3400.687;Inherit;False;Constant;_Vector1;Vector 1;14;0;Create;True;0;0;0;False;0;False;0,0,0;0,0,0;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;639;-2670.66,3587.948;Inherit;False;2;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.DynamicAppendNode;640;-2430.399,3582.595;Inherit;True;FLOAT4;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.ClampOpNode;641;-2410.373,3232.055;Inherit;False;3;0;FLOAT;0;False;1;FLOAT;10;False;2;FLOAT;10;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;643;-2167.6,3471.973;Inherit;True;3;0;FLOAT4;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;2;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.RangedFloatNode;644;-2209.584,3387.691;Inherit;False;Property;_WindDirection;Wind Direction;21;0;Create;True;0;0;0;False;0;False;1;1.5678;1.54;1.6;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleAddOpNode;645;-1899.668,3582.209;Inherit;True;2;2;0;FLOAT4;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.RotateAboutAxisNode;646;-1569.403,3470.413;Inherit;False;False;4;0;FLOAT3;0,0,0;False;1;FLOAT;0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.WorldPosInputsNode;625;-4048.481,3498.896;Inherit;False;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.GetLocalVarNode;642;-2103.829,3702.54;Inherit;False;344;FinalWind;1;0;OBJECT;;False;1;FLOAT4;0
Node;AmplifyShaderEditor.StaticSwitch;441;-1245.784,3522.152;Inherit;False;Property;_WindOnOff;Wind On/Off;28;0;Create;True;0;0;0;False;0;False;0;1;1;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RegisterLocalVarNode;439;-1026.048,3475.47;Inherit;False;TrunkPivotBend;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.PosVertexDataNode;624;-3132.612,3379.036;Inherit;False;0;0;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;636;-2808.045,3252.728;Inherit;False;Constant;_Float1;Float 1;14;0;Create;True;0;0;0;False;0;False;10;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.WireNode;649;-3685.029,3457.204;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.PosVertexDataNode;287;-3772.51,4119.935;Inherit;False;0;0;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.SamplerNode;426;-2960.128,-359.7853;Inherit;True;Property;_AlbedoMap;Albedo Map;0;2;[Header];[NoScaleOffset];Create;True;3;_____________(TFS) TREE LEAF SHADER_______________;_____________________________________________________;Texture Maps;0;0;True;0;False;-1;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.FunctionNode;395;-4183.667,-435.2303;Inherit;False;Random Range;-1;;18;7b754edb8aebbfb4a9ace907af661cfc;0;3;1;FLOAT2;10,4;False;2;FLOAT;0.9;False;3;FLOAT;1.15;False;1;FLOAT;0
Node;AmplifyShaderEditor.TransformPositionNode;653;-4397.545,-445.3751;Inherit;False;Object;World;False;Fast;True;1;0;FLOAT3;0,0,0;False;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
WireConnection;393;0;416;0
WireConnection;393;1;431;0
...
WireConnection;409;1;415;3
WireConnection;409;2;426;0
WireConnection;409;3;410;1
WireConnection;409;4;410;2
WireConnection;411;0;409;0
WireConnection;411;1;472;0
WireConnection;416;0;415;2
...
WireConnection;471;0;595;0
WireConnection;412;0;433;0
WireConnection;414;0;411;0
WireConnection;414;1;373;0
WireConnection;433;1;373;0
WireConnection;433;0;414;0
...
WireConnection;375;0;396;0
WireConnection;372;1;386;0
WireConnection;372;0;367;0
WireConnection;610;0;475;0
WireConnection;610;1;421;0
WireConnection;610;7;424;0
WireConnection;610;8;425;0
WireConnection;610;9;423;0
WireConnection;610;10;427;0
WireConnection;610;31;622;0
WireConnection;610;32;623;0
WireConnection;610;11;438;0
WireConnection;610;12;449;0
...
WireConnection;628;0;627;1
WireConnection;628;1;627;3
...
WireConnection;635;0;624;2
WireConnection;635;1;633;0
WireConnection;637;0;649;0
WireConnection;637;1;636;0
WireConnection;639;0;635;0
WireConnection;639;1;634;0
WireConnection;640;0;639;0
WireConnection;640;1;638;2
WireConnection;641;0;637;0
WireConnection;643;0;638;2
WireConnection;643;1;640;0
WireConnection;643;2;641;0
WireConnection;645;0;642;0
WireConnection;645;1;643;0
WireConnection;646;0;353;0
WireConnection;646;1;644;0
WireConnection;646;3;645;0
WireConnection;441;0;646;0
WireConnection;439;0;441;0
WireConnection;649;0;627;2
WireConnection;363;0;383;0
WireConnection;363;1;368;0
WireConnection;363;2;370;0
WireConnection;395;1;653;0
ASEEND*/
//CHKSM=EE505A13D606F4FE9CA89270EDA8B8F3DFD78F82